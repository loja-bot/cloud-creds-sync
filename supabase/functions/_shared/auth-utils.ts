...
export async function checkRateLimit(
  identifier: string, action: string, maxAttempts: number, windowMinutes: number,
): Promise<boolean> {
  ...
  const { count } = await supa
    ...
    .eq('identifier', identifier).eq('action', action).gte('attempt_at', since);
...
export async function sendCodeEmail(to: string, code: string) {
  ...
  const nm = await import('npm:nodemailer@6.9.14');
  ...
  await transporter.sendMail({
    ...
    text: `Seu código: ${code} (expira em 15 minutos)`,
    html,
  });
}
